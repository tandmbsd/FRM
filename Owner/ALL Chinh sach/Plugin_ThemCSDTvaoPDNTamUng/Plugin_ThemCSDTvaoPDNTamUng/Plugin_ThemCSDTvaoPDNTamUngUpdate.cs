...
{
    public sealed class Plugin_ThemCSDTvaoPDNTamUngUpdate : IPlugin
    {
        ...
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
            ...
            if (context.InputParameters.Contains("Target") && (context.InputParameters["Target"] is Entity))
            {
                ...
                if (ChiTietPDNTamUng.Contains("new_thuadatcanhtac"))
                {
                    ...
                    if (ChiTietPDNTamUng.LogicalName == "new_chitietphieudenghitamung")
                    {
                        ...
                        {
                            ChiTietPDNTamUng = service.Retrieve("new_chitietphieudenghitamung", entityId, new ColumnSet(new string[] { "new_loaihopdong", "new_hopdongdautumia", "new_chitiethddtmia", "new_hopdongdaututhuedat", "new_chitiethddtthuedat", "createdon", "new_hopdongdaututrangthietbi", "new_chitiethddttrangthietbi", "new_phieudenghitamung", "new_phantramtamung" }));
                            ...
                            if (ChiTietPDNTamUng.Contains("new_loaihopdong") && ChiTietPDNTamUng.GetAttributeValue<OptionSetValue>("new_loaihopdong").Value.ToString() == "100000000")  // HDDT Mía
                            {
                                if (ChiTietPDNTamUng.Contains("new_hopdongdautumia") && ChiTietPDNTamUng.Contains("new_chitiethddtmia"))
                                {
                                    ...
                                    Entity HDDTmia = service.Retrieve("new_hopdongdautumia", HDDTmiaId, new ColumnSet(new string[] { "new_vudautu", "new_tongdinhmucdautu", "new_tongtiendautu" }));
                                    ...
                                    else
                                    {
                                        ...
                                        if (resultCol != null && resultCol.Entities.Count > 0)
                                        {
                                            ...
                                            if (mCSDT != null && mCSDT.Id != Guid.Empty)
                                            {
                                                ...
                                                decimal tamungtt = (ChiTietPDNTamUng.Contains("new_phantramtamung") ? (decimal)ChiTietPDNTamUng["new_phantramtamung"] : 0);

                                                // Tinh Đinh muc Tạm  ứng
                                                decimal tongTienDT = (HDDTmia.Contains("new_tongtiendautu") ? HDDTmia.GetAttributeValue<Money>("new_tongtiendautu").Value : 0) * tamungtt / 100;
                            ...
                            if (ChiTietPDNTamUng.Contains("new_loaihopdong") && ChiTietPDNTamUng.GetAttributeValue<OptionSetValue>("new_loaihopdong").Value.ToString() == "100000001")  // // HDDT thuê đất
                            {
                                if (ChiTietPDNTamUng.Contains("new_hopdongdaututhuedat") && ChiTietPDNTamUng.Contains("new_chitiethddtthuedat"))
                                {
                                    ...
                                    Entity HDDTThuedat = service.Retrieve("new_hopdongthuedat", DHDTtdId, new ColumnSet(new string[] { "new_vudautu", "new_tongdinhmucdautu", "new_tongtienthuedat" }));
                                    ...
                                    else
                                    {
                                        ...
                                        if (result != null && result.Entities.Count > 0)
                                        {
                                            ...
                                            if (mCSDT != null && mCSDT.Id != Guid.Empty)
                                            {
                                                ...
                                                // Tinh Đinh muc Tạm  ứng
                                                decimal tamungtt = (ChiTietPDNTamUng.Contains("new_phantramtamung") ? (decimal)ChiTietPDNTamUng["new_phantramtamung"] : 0);
                                                decimal tongDMDT = (HDDTThuedat.Contains("new_tongtienthuedat") ? HDDTThuedat.GetAttributeValue<Money>("new_tongtienthuedat").Value : 0) * tamungtt / 100;
                                    ...
                                    else
                                    {
                                        ...
                                        if (resultTTB != null && resultTTB.Entities.Count > 0)
                                        {
                                            ...
                                            if (mCSDT != null && mCSDT.Id != Guid.Empty)
                                            {
                                                ...
                                                // Tinh Đinh muc Tạm  ứng
                                                decimal tongDMDT = (hddtTTBObj.Contains("new_tonggiatrihopdong") ? hddtTTBObj.GetAttributeValue<Money>("new_tonggiatrihopdong").Value : 0) * tamungtt / 100;
                                                Money MtongDMDT = new Money(tongDMDT);
                                                en["new_dinhmuctamung"] = MtongDMDT;
