    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (service = new OrganizationService(serverConnection.CRMConnection))
            {
                ...
                Entity thuadatObj = service.Retrieve("new_thuadat", ThuadatRef.Id, new ColumnSet(new string[] { "new_nhomdat", "new_loaisohuudat", "new_vungdialy", "new_nhomculy" }));

                EntityReference NTTDref = ChiTietNTTD.GetAttributeValue<EntityReference>("new_nghiemthuthuedat");
                Entity NTTD = service.Retrieve("new_nghiemthuthuedat", NTTDref.Id, new ColumnSet(new string[] { "new_hopdongdaututhuedat", "new_datthue", "new_lannghiemthu_global" }));
                ...
                if (ctHDTDTD != null)
                {
                    ...
                    if (mCSDT != null && mCSDT.Id != Guid.Empty)
                    {
                        ...
                        en["new_sotiendautu"] = new Money(tienHL * (ChiTietNTTD.Contains("new_dientichnghiemthu") ? (decimal)ChiTietNTTD["new_dientichnghiemthu"] : 0));
