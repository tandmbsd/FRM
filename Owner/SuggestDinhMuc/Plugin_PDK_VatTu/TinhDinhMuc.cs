...
namespace Plugin_PDK_VatTu
{
    public class TinhDinhMuc
    {
        ...
        public void CalculateTrongMia()
        {
            ...
            if (pdk == null)
                throw new Exception(string.Format("Phiếu đăng ký vật tư '{0}' không tồn tại hoặc bị xóa!", pdkRef.Name));
            if (!pdk.Contains("new_hopdongdautumia"))
                throw new Exception(string.Format("Vui lòng chọn hợp đầu tư mía cho phiếu đăng ký vật tư '{0}'!", pdkRef.Name));
            string pdk_name = pdk.Contains("new_name") ? (" '" + pdk["new_name"].ToString() + "'") : "";

            if (!pdk.Contains("new_masophieudangky"))
                throw new Exception(string.Format("Phiếu đăng ký vật tư{0} chưa có mã. Vui lòng cập nhật mã!", pdk_name));
            ...
            QueryExpression query = new QueryExpression("new_thuadat_pdkvattukhac");
            ...
            if (((OptionSetValue)pdk["new_apdung"]).Value == 100000000)
            {
                ...
                foreach (Entity cthd in cthds.Entities)
                {
                    ...
                    {
                        throw new Exception(cthd["new_name"].ToString() + " không có chính sách đầu tư");
                    }
                    ...
                    Entity ct_pdk = new Entity("new_thuadat_pdkvattukhac");
        ...
        private void sum_pdkNT(EntityReference hd, EntityReference pdkRef, ref decimal hlTM,
            ref decimal hlVT, ref decimal KHL, int type, string fieldNT, Guid pNT)
        {
            decimal sumhlTM = 0;
            decimal sumhlVT = 0;
            decimal sumKHL = 0;
