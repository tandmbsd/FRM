...
namespace Action_CopyChinhSachThuMua
{
    public class Action_CopyChinhSachThuMua : IPlugin
    {
        ...
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
            ...
            if (target.LogicalName == "new_chinhsachthumua")
            {
                factory = (IOrganizationServiceFactory)serviceProvider.GetService(typeof(IOrganizationServiceFactory));
                service = factory.CreateOrganizationService(context.UserId);
                Entity CSTM = service.Retrieve(target.LogicalName, target.Id, new ColumnSet(true));
                if (CSTM == null)
                    throw new Exception("Chính sách thu mua này không tồn tại!");

                //Guid entityId = CSTM.Id;

                traceService.Trace("Begin Action");
                ...
                if (newvuDT != null && newvuDT.Id != Guid.Empty)
                {
                    ...
                    newCSTM["new_miadonga"] = (CSTM.Contains("new_miadonga") ? (bool)CSTM["new_miadonga"] : false);     // mía đổ ngã
                    ...
                    newCSTM["new_congdieutunoikhac"] = (CSTM.Contains("new_congdieutunoikhac") ? (bool)CSTM["new_congdieutunoikhac"] : false);      // công điều từ nơi khác
                    ...
                    newCSTM["new_hoanthanhhopdong"] = (CSTM.Contains("new_hoanthanhhopdong") ? (bool)CSTM["new_hoanthanhhopdong"] : false);    // hoàn thành hợp đồng
                    ...
                    // Hỗ trợ  
                    if (CSTM.Attributes.Contains("new_dongiahotromuamia"))   // dơn giá hỗ trợ mua mía
                    {
                        Money dgHoTroMuamia = (Money)CSTM["new_dongiahotromuamia"];
                        newCSTM["new_dongiahotromuamia"] = dgHoTroMuamia;
                    }
                    if (CSTM.Attributes.Contains("new_dongiahotrovanchuyen"))   // dơn giá hỗ trợ vận chuyển
                    {
                    ...
                    // Chính sách thu mua tạp chất thưởng
                    ...
                    context.OutputParameters["ReturnId"] = newCSTMID.ToString();
                } //if (newvuDT != null && newvuDT.Id != Guid.Empty)  
                else
                {
