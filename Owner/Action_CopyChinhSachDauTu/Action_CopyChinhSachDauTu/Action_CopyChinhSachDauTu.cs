...
{
    public class Action_CopyChinhSachDauTu : IPlugin
    {
        ...
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
            ...
            if (target.LogicalName == "new_chinhsachdautu")
            {
                factory = (IOrganizationServiceFactory)serviceProvider.GetService(typeof(IOrganizationServiceFactory));
                service = factory.CreateOrganizationService(context.UserId);
                Entity CSDT = service.Retrieve(target.LogicalName, target.Id, new ColumnSet(true));
                if (CSDT == null)
                    throw new Exception("Chính sách đầu tư này không tồn tại!");

                //Guid entityId = CSDT.Id;
                ...
                if (newvuDT != null && newvuDT.Id != Guid.Empty)
                {
                    ...
                    newCSDT["new_bonphan"] = (CSDT.Contains("new_bonphan") ? (bool)CSDT["new_bonphan"]: false);     // bon phan
                    newCSDT["new_trongmia"] = (CSDT.Contains("new_trongmia") ? (bool)CSDT["new_trongmia"] : false); // trong mia
                    newCSDT["new_tuoi"] = (CSDT.Contains("new_tuoi") ? (bool)CSDT["new_tuoi"] : false);             // tuoi  
                    newCSDT["new_thuhoach"] = (CSDT.Contains("new_thuhoach") ? (bool)CSDT["new_thuhoach"] : false); // thu hoach
                    newCSDT["new_chamsoc"] = (CSDT.Contains("new_chamsoc") ? (bool)CSDT["new_chamsoc"] : false);    // cham soc
                    ...
                    if (LangiainganCol != null && LangiainganCol.Entities.Count > 0)
                    {
                        foreach (Entity a in LangiainganCol.Entities)
                        {
                            ...
                            langiaingan["new_yeucau"] = (OptionSetValue)a["new_yeucau"];
                    ...
                    if (TileTHVCol != null && TileTHVCol.Entities.Count > 0)
                    {
                        foreach (Entity a in TileTHVCol.Entities)
                        {
                            ...
                            tileTHV["new_vuthuhoi"] = a.Contains("new_vuthuhoi") ? a["new_vuthuhoi"] : null;
                    ...
                    context.OutputParameters["ReturnId"] = newCSDTID.ToString();
                } //if(newvuDT !=null && newvuDT.Id != Guid.Empty)
                else
                {
