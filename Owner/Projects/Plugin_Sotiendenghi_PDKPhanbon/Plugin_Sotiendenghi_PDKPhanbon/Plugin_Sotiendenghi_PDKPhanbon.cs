...
namespace Plugin_Sotiendenghi_PDKPhanbon
{
    public class Plugin_Sotiendenghi_PDKPhanbon : IPlugin
    {
        ...
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
            ...
            if (context.MessageName.ToUpper() == "CREATE")
            {
                ...
                Entity chitietpdkphanbon = service.Retrieve(target.LogicalName, target.Id, new ColumnSet(true));

                if (!chitietpdkphanbon.Contains("new_phieudangkyphanbon"))
                {
                    throw new Exception("Chi tiết pdk phân bón không có phiếu đăng ký phân bón !!! ");
                }

                Entity pdkphanbon = service.Retrieve("new_phieudangkyphanbon", ((EntityReference)chitietpdkphanbon["new_phieudangkyphanbon"]).Id, new ColumnSet(true));
                ...
                List<Entity> listCTPDKPB = RetrieveMultiRecord(service, "new_chitietdangkyphanbon", new ColumnSet(true), "new_phieudangkyphanbon"
                    , pdkphanbon.Id);

                foreach (Entity ct in listCTPDKPB)
                {
                ...
                EntityCollection listCTHD = RetrieveNNRecord(service, "new_thuadatcanhtac", "new_phieudangkyphanbon", "new_new_pdkphanbon_new_chitiethddtmia", 
                    new ColumnSet(new string[] { "new_thuadatcanhtacid" }), "new_phieudangkyphanbonid", pdkphanbon.Id);
                if (listCTHD != null && listCTHD.Entities.Count > 0)
                {
                    ...
                    foreach (Entity cthd in listCTHD.Entities)
                    {
                        ...
                        DateTime ngaytao = chitietpdkphanbon.GetAttributeValue<DateTime>("createdon");
                ...
                Entity newPDKPhanbon = new Entity("new_phieudangkyphanbon");
                newPDKPhanbon.Id = pdkphanbon.Id;

                newPDKPhanbon["new_dinhmuc_khonghoanlai"] = new Money(TongDMDTKHL + (pdkphanbon.Contains("new_dinhmuc_khonghoanlai") ? ((Money)pdkphanbon["new_dinhmuc_khonghoanlai"]).Value : 0));
                newPDKPhanbon["new_dinhmuc_hoanlai_vattu"] = new Money(TongDMDTHLvattu + (pdkphanbon.Contains("new_dinhmuc_hoanlai_vattu") ? ((Money)pdkphanbon["new_dinhmuc_hoanlai_vattu"]).Value : 0));
                newPDKPhanbon["new_dinhmuc_hoanlai_tienmat"] = new Money(TongDMDTHL + (pdkphanbon.Contains("new_dinhmuc_hoanlai_tienmat") ? ((Money)pdkphanbon["new_dinhmuc_hoanlai_tienmat"]).Value : 0));

                newPDKPhanbon["new_giaingan_khonghoanlai"] = new Money(daGiainganKHL + (pdkphanbon.Contains("new_giaingan_khonghoanlai") ? ((Money)pdkphanbon["new_giaingan_khonghoanlai"]).Value : 0));
                newPDKPhanbon["new_giaingan_hoanlai_vattu"] = new Money(daGiainganHLvattu + (pdkphanbon.Contains("new_giaingan_hoanlai_vattu") ? ((Money)pdkphanbon["new_giaingan_hoanlai_vattu"]).Value : 0));
                newPDKPhanbon["new_giaingan_hoanlai_tienmat"] = new Money(0);

                newPDKPhanbon["new_denghi_khonghoanlai"] = new Money(DenghiKHL + (pdkphanbon.Contains("new_denghi_khonghoanlai") ? ((Money)pdkphanbon["new_denghi_khonghoanlai"]).Value : 0));
                newPDKPhanbon["new_denghi_hoanlai_vattu"] = new Money(DenghiHLvattu + (pdkphanbon.Contains("new_denghi_hoanlai_vattu") ? ((Money)pdkphanbon["new_denghi_hoanlai_vattu"]).Value : 0));

                service.Update(newPDKPhanbon);
            }
            #endregion
            #region Update
            if (context.MessageName.ToUpper() == "UPDATE")
            {
                ...
                if (target.Contains("new_soluong"))
                {
                    ...
                    List<Entity> listCTPDKPB = RetrieveMultiRecord(service, "new_chitietdangkyphanbon", new ColumnSet(true)
                        , "new_phieudangkyphanbon", pdkphanbon.Id);
                    ...
                    EntityCollection listCTHD = RetrieveNNRecord(service, "new_thuadatcanhtac", "new_phieudangkyphanbon", "new_new_pdkphanbon_new_chitiethddtmia"
                        , new ColumnSet(true), "new_phieudangkyphanbonid", pdkphanbon.Id);
                    if (listCTHD != null && listCTHD.Entities.Count > 0)
                    {
                        #region for listCTHD
                        foreach (Entity cthd in listCTHD.Entities)
