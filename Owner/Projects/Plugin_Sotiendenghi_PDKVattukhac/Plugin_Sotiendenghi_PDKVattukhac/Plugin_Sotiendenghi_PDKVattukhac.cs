...
namespace Plugin_Sotiendenghi_PDKVattukhac
{
    public class Plugin_Sotiendenghi_PDKVattukhac : IPlugin
    {
        ...
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
            ...
            if (context.MessageName.ToUpper() == "CREATE")
            {
                ...
                Entity chitietpdkvattu = service.Retrieve(target.LogicalName, target.Id, new ColumnSet(true));

                if (!chitietpdkvattu.Contains("new_phieudangkyvattu"))
                {
                    throw new Exception("Chi tiết pdk vật tư không có phiếu đăng ký vật tư !!! ");
                }

                Entity pdkvattu = service.Retrieve("new_phieudangkyvattu", ((EntityReference)chitietpdkvattu["new_phieudangkyvattu"]).Id, new ColumnSet(true));
                ...
                List<Entity> listCTPDKVT = RetrieveMultiRecord(service, "new_chitietdangkyvattu", new ColumnSet(true), "new_phieudangkyvattu", pdkvattu.Id);

                foreach (Entity ct in listCTPDKVT)
                {
                ...
                EntityCollection listCTHD = RetrieveNNRecord(service, "new_thuadatcanhtac", "new_phieudangkyvattu", "new_new_pdkvattu_new_chitiethddtmia", new ColumnSet(new string[] { "new_thuadatcanhtacid" }), "new_phieudangkyvattuid", pdkvattu.Id);
                if (listCTHD != null && listCTHD.Entities.Count > 0)
                {
                    ...
                    foreach (Entity cthd in listCTHD.Entities)
                    {
                        ...
                        DateTime ngaytao = chitietpdkvattu.GetAttributeValue<DateTime>("createdon");
                ...
                Entity newpdkvattu = new Entity("new_phieudangkyvattu");
                newpdkvattu.Id = pdkvattu.Id;

                newpdkvattu["new_dinhmuc_khonghoanlai"] = new Money(TongDMDTKHL + (pdkvattu.Contains("new_dinhmuc_khonghoanlai") ? ((Money)pdkvattu["new_dinhmuc_khonghoanlai"]).Value : 0));
                newpdkvattu["new_dinhmuc_hoanlai_vattu"] = new Money(TongDMDTHLvattu + (pdkvattu.Contains("new_dinhmuc_hoanlai_vattu") ? ((Money)pdkvattu["new_dinhmuc_hoanlai_vattu"]).Value : 0));
                newpdkvattu["new_dinhmuc_hoanlai_tienmat"] = new Money(TongDMDTHL + (pdkvattu.Contains("new_dinhmuc_hoanlai_tienmat") ? ((Money)pdkvattu["new_dinhmuc_hoanlai_tienmat"]).Value : 0));

                newpdkvattu["new_giaingan_khonghoanlai"] = new Money(daGiainganKHL + (pdkvattu.Contains("new_giaingan_khonghoanlai") ? ((Money)pdkvattu["new_giaingan_khonghoanlai"]).Value : 0));
                newpdkvattu["new_giaingan_hoanlai_vattu"] = new Money(daGiainganHLvattu + (pdkvattu.Contains("new_giaingan_hoanlai_vattu") ? ((Money)pdkvattu["new_giaingan_hoanlai_vattu"]).Value : 0));
                newpdkvattu["new_giaingan_hoanlai_tienmat"] = new Money(0);

                newpdkvattu["new_denghi_khonghoanlai"] = new Money(DenghiKHL + (pdkvattu.Contains("new_denghi_khonghoanlai") ? ((Money)pdkvattu["new_denghi_khonghoanlai"]).Value : 0));
                newpdkvattu["new_denghi_hoanlai_vattu"] = new Money(DenghiHLvattu + (pdkvattu.Contains("new_denghi_hoanlai_vattu") ? ((Money)pdkvattu["new_denghi_hoanlai_vattu"]).Value : 0));
            ...
            if (context.MessageName.ToUpper() == "UPDATE")
            {
                ...
                if (target.Contains("new_soluong"))
                {
                    ...
                    List<Entity> listCTPDKT = RetrieveMultiRecord(service, "new_chitietdangkyvattu", new ColumnSet(true), "new_phieudangkyvattu", pdkvattu.Id);
                    ...
                    EntityCollection listCTHD = RetrieveNNRecord(service, "new_thuadatcanhtac", "new_phieudangkyvattu", "new_new_pdkvattu_new_chitiethddtmia", new ColumnSet(true), "new_phieudangkyvattuid", pdkvattu.Id);
                    if (listCTHD != null && listCTHD.Entities.Count > 0)
                    {
                        #region for listCTHD
                        foreach (Entity cthd in listCTHD.Entities)
