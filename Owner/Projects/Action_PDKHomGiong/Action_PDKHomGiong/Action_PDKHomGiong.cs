...
namespace Action_PDKHomGiong
{
    public class Action_PDKHomGiong : IPlugin
    {
        ...
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
            ...
            if (context.MessageName.ToUpper() == "CREATE")
            {
                ...
                Entity chitietpdkhomgiong = service.Retrieve(target.LogicalName, target.Id, new ColumnSet(true));

                if (!chitietpdkhomgiong.Contains("new_phieudangkyhomgiong"))
                {
                ...
                List<Entity> listCTPDKHG = RetrieveMultiRecord(service, "new_chitietdangkyhomgiong", new ColumnSet(true), "new_phieudangkyhomgiong", pdkhomgiong.Id);

                foreach (Entity ct in listCTPDKHG)
                {
                ...
                EntityCollection listCTHD = RetrieveNNRecord(service, "new_thuadatcanhtac", "new_phieudangkyhomgiong", "new_new_pdkhomgiong_new_chitiethddtmia", new ColumnSet(new string[] { "new_thuadatcanhtacid" }), "new_phieudangkyhomgiongid", pdkhomgiong.Id);
                if (listCTHD != null && listCTHD.Entities.Count > 0)
                {
                    ...
                    foreach (Entity cthd in listCTHD.Entities)
                    {
                        ...
                        DateTime ngaytao = chitietpdkhomgiong.GetAttributeValue<DateTime>("createdon");
                ...
                Entity newPDKHomgiong = new Entity("new_phieudangkyhomgiong");
                newPDKHomgiong.Id = pdkhomgiong.Id;

                newPDKHomgiong["new_dinhmuc_khonghoanlai"] = new Money(TongDMDTKHL + (pdkhomgiong.Contains("new_dinhmuc_khonghoanlai") ? ((Money)pdkhomgiong["new_dinhmuc_khonghoanlai"]).Value : 0));
                newPDKHomgiong["new_dinhmuc_hoanlai_vattu"] = new Money(TongDMDTHLvattu + (pdkhomgiong.Contains("new_dinhmuc_hoanlai_vattu") ? ((Money)pdkhomgiong["new_dinhmuc_hoanlai_vattu"]).Value : 0));
                newPDKHomgiong["new_dinhmuc_hoanlai_tienmat"] = new Money(TongDMDTHL + (pdkhomgiong.Contains("new_dinhmuc_hoanlai_tienmat") ? ((Money)pdkhomgiong["new_dinhmuc_hoanlai_tienmat"]).Value : 0));

                newPDKHomgiong["new_giaingan_khonghoanlai"] = new Money(daGiainganKHL + (pdkhomgiong.Contains("new_giaingan_khonghoanlai") ? ((Money)pdkhomgiong["new_giaingan_khonghoanlai"]).Value : 0));
                newPDKHomgiong["new_giaingan_hoanlai_vattu"] = new Money(daGiainganHLvattu + (pdkhomgiong.Contains("new_giaingan_hoanlai_vattu") ? ((Money)pdkhomgiong["new_giaingan_hoanlai_vattu"]).Value : 0));
                newPDKHomgiong["new_giaingan_hoanlai_tienmat"] = new Money(0);

                newPDKHomgiong["new_denghi_khonghoanlai"] = new Money(DenghiKHL + (pdkhomgiong.Contains("new_denghi_khonghoanlai") ? ((Money)pdkhomgiong["new_denghi_khonghoanlai"]).Value : 0));
                newPDKHomgiong["new_denghi_hoanlai_vattu"] = new Money(DenghiHLvattu + (pdkhomgiong.Contains("new_denghi_hoanlai_vattu") ? ((Money)pdkhomgiong["new_denghi_hoanlai_vattu"]).Value : 0));

                service.Update(newPDKHomgiong);
            }
            #endregion
            #region Update
            if (context.MessageName.ToUpper() == "UPDATE")
            {
                ...
                if (target.Contains("new_soluong"))
                {
                    ...
                    EntityCollection listCTHD = RetrieveNNRecord(service, "new_thuadatcanhtac", "new_phieudangkyhomgiong", "new_new_pdkhomgiong_new_chitiethddtmia", new ColumnSet(true), "new_phieudangkyhomgiongid", pdkhomgiong.Id);
                    if (listCTHD != null && listCTHD.Entities.Count > 0)
                    {
                        #region for listCTHD
                        foreach (Entity cthd in listCTHD.Entities)
