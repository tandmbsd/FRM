...
{
    public sealed partial class Workflow_TinhLichThuHoach10Ngay : CodeActivity
    {
        [RequiredArgument]
        [Input("Vụ thu hoạch")]
        [ReferenceTarget("new_vuthuhoach")]
        ...
        protected override void Execute(CodeActivityContext executionContext)
        {
            ...
            if (Vuthuhoach != null)
            {
                ...
                if (Vuthuhoach != null)
                {
                    ...
                    foreach (Entity a in doanth.Entities)
                    {
                        ...
                        QueryExpression qettut = new QueryExpression("new_thutuuutienlydothuhoachsom");
                        qettut.ColumnSet = new ColumnSet(new string[] { "new_stt", "new_lydo" });
                        qettut.Criteria.AddCondition(new ConditionExpression("new_giaidoanthuhoach", ConditionOperator.Equal, a.Id));

                        EntityCollection ttlydouutien = service.RetrieveMultiple(qettut);
                        foreach (Entity l in ttlydouutien.Entities)
                            tmp.thutuuutienTHSom.Add(((EntityReference)l["new_lydo"]).Id, (int)l["new_stt"]);
                ...
                qe.ColumnSet = new ColumnSet(new string[] { "new_ngaytrong", "new_tram", "new_khachhang", "new_khachhangdoanhnghiep", "new_ngaythuhoachdukien", "new_ngaydukienthuhoachsom", "new_lydothuhoachsom", "new_sanluonguoc", "new_loaigocmia", "new_luugoc", "new_ngayktbonphan", "new_tuoimia" });
                    ...
                    foreach (Entity a in result.Entities)
                    {
                        if (a.Contains("new_tram") && a.Contains("new_ngaythuhoachdukien") && (a.Contains("new_khachhang") || a.Contains("new_khachhangdoanhnghiep")))
                        {
                            ...
                            Thuadat tmp = new Thuadat()
                            {
                                ThuaId = a.Id,
                                KhachHangId = KHId,
                                NgayTHDK = ((a.Contains("new_ngaydukienthuhoachsom") ? (DateTime)a["new_ngaydukienthuhoachsom"] : (a.Contains("new_ngaydukienthuhoach") ? (DateTime)a["new_ngaydukienthuhoach"] : (DateTime)a["new_ngaythuhoachdukien"]))).AddHours(7).Date,
                            ...
                            if (a.Contains("new_lydothuhoachsom"))
                            {
                                tmp.Lydothsom = ((EntityReference)a["new_lydothuhoachsom"]).Id;
                                //tmp.CTBBThuhoachsom = ((EntityReference)a["new_chitietbbthuhoachsomid"]).Id;
                                tmp.flag = 0;
                            }
                ...
                foreach (KeyValuePair<Guid, TramTH> a in DSTramTH)
                {
                    foreach (KeyValuePair<Guid, DoanTH> b in DSDoanTH)
                    {
                        ...
                        {
                            foreach (Thuadat l in a.Value.dsthua)
                            {
                                if (l.Lydothsom != Guid.Empty)
                                {
                                    int value = 2147483647;
                                    if (b.Value.thutuuutienTHSom.TryGetValue(l.Lydothsom, out value))
                                        l.orderTHS = value;
                                }
                            }
                            ...
                            a.Value.dsthua = listsoft.ThenBy(o => o.orderTHS).ToList();
                            preGD = b.Key;
                        }
                        ...
                        foreach (KeyValuePair<Guid, DotTH> c in b.Value.dotth)
                        {
                            ...
                            if (quotaDot > 0)
                            {
                                ...
                                if (tongQuota < quotaDot)
                                    DSThua = GetThuaDenHan(binhquanThua, c.Value.den, quotaDot, a.Value.dsthua, ref tongQuota, ref countThua, ref countKH, ref QuotaKH, ref bqVuot);
                                if (DSThua.Count > 0)
                                {
                                    binhquanThua = bqVuot;
            ...
            {
                throw new Exception("Không tìm thấy Vụ thu hoạch, vui lòng save form trước khi chạy tính năng !");
            }
        }

        public static List<Thuadat> GetThuaDenHan(int bqThua, DateTime denngay, List<Thuadat> source, ref int tongquota, ref int countThua, ref int countKH, ref Dictionary<Guid, int> daPhan, ref int bqVuot)
        {
            decimal tongsl = 0;
            decimal tonglonbq = 0;
            int countlonbq = 0;
        ...
        public static List<Thuadat> GetThuaDenHan(int bqThua, DateTime denngay, decimal quota, List<Thuadat> source, ref int tongquota, ref int countThua, ref int countKH, ref Dictionary<Guid, int> daPhan, ref int bqVuot)
        {
            decimal tongsl = 0;
            decimal tonglonbq = 0;
            int countlonbq = 0;
            ...
            foreach (Thuadat a in source)
            {
                if (a.Sanluong > 0)
                {
                    ...
                    dsKH.Add(a.KhachHangId);
                    dsThua.Add(a.ThuaId);
                    if (a.flag == 0 && a.NgayTHDK <= denngay)
                    {
                        result.Insert(flagcount, a);
                        flagcount++;
                    }
