...
namespace Plugin_ThuaCanhTac_GenQuyTrinhCanhTac
{
    public class GenQuyTrinhCanhTac : IPlugin
    {
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            Entity target = null;
            if (context.InputParameters["Target"].GetType().Name == "EntityReference")
            {
                EntityReference ref_target = context.InputParameters["Target"] as EntityReference;
                target = new Entity(ref_target.LogicalName);
                target.Id = ref_target.Id;
            }
            else
            ...
            if (target.LogicalName == "new_thuadatcanhtac")
            {
                ...
                if (context.MessageName == "Create" && (target.Contains("new_ngaytrongdukien") || target.Contains("new_ngaytrong")))
                {
                    if (!target.Contains("new_phuluchopdongid"))
                    {
                        factory = (IOrganizationServiceFactory)serviceProvider.GetService(typeof(IOrganizationServiceFactory));
                        service = factory.CreateOrganizationService(context.UserId);
                        Gen(target, true, false);
                    }
                ...
                else if (context.MessageName == "Update")
                {
                    ...
                    if (target.Contains("new_ngaytrong"))
                    {
                        bool flag = false;
                    ...
                    else
                    {
                        ...
                        if (flag == true || target.Contains("new_ngaytrongdukien"))
                        {
                            factory = (IOrganizationServiceFactory)serviceProvider.GetService(typeof(IOrganizationServiceFactory));
                            service = factory.CreateOrganizationService(context.UserId);
                            Entity chitiethddtm = service.Retrieve(target.LogicalName, target.Id, new ColumnSet(true));
                        ...
                        {
                            {
                                ...
                                en["scheduledstart"] = new_ngaytrong.AddDays((int)qtc["new_songaysaukhitrong"]);
                                en["new_dientich"] = hopdongdautumia.Contains("new_dientichhopdong") ? hopdongdautumia["new_dientichhopdong"] : new decimal(0);
                                //if (qtc.Attributes.Contains("new_sothoigianthuchien"))
                                //    en["new_ngayxulythucte"] = qtc["new_sothoigianthuchien"];
                                en["regardingobjectid"] = target.ToEntityReference();
                                service.Create(en);
