        ...
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
            ...
            if (context.MessageName == "Create")
            {
                ...
                if (target.LogicalName == "new_chitietdangkythuoc" && target.Contains("new_phieudangkythuoc"))
                {
                    ...
                    EntityReference pdkRef = (EntityReference)target["new_phieudangkythuoc"];
            ...
            else if (context.MessageName == "Update")
            {
                ...
                if (target.LogicalName == "new_chitietdangkythuoc")
                {
                    ...
                    EntityReference pdkRef = (EntityReference)fullEntity["new_phieudangkythuoc"];
                ...
                {
                    int statuscode = ((OptionSetValue)target["new_tinhtrangduyet"]).Value;
                    if (statuscode != 100000002)//da duyet
                    {
                        factory = (IOrganizationServiceFactory)serviceProvider.GetService(typeof(IOrganizationServiceFactory));
                        service = factory.CreateOrganizationService(context.UserId);
                        trace = serviceProvider.GetService(typeof(ITracingService)) as ITracingService;
