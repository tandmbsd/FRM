...
{
    public sealed class Plugin_ThemCSDTvaoNTBocLaMia : IPlugin
    {
                    ...
                    if (context.MessageName.ToUpper() == "CREATE")
                    {
                        ...
                        else
                        {
                            ...
                            if (ChiTietNTBocLaMia.Contains("new_chitiethddtmia"))
                            {
                                ...
                                ctHDDTmia = service.Retrieve("new_thuadatcanhtac", ctHDDTmiaId, new ColumnSet(new string[] { "new_vutrong", "new_loaigocmia", "new_mucdichsanxuatmia", "new_giongmia", "new_thuadat", "createdon", "new_hopdongdautumia", "new_khachhang", "new_khachhangdoanhnghiep", "new_thamgiamohinhkhuyennong", "new_dientichthucte" }));
                            }
                            else
                            ...
                            if (CSDT != null && CSDT.Count() > 0)
                            {
                                foreach (Entity a in CSDT)
                                {
                                    ...
                                    {
                                        if (ChiTietNTBocLaMia.Contains("new_vutrong"))
                                        {
                                            if (a["new_vutrong_vl"].ToString().IndexOf(((OptionSetValue)ChiTietNTBocLaMia["new_vutrong"]).Value.ToString()) == -1)
                                                continue;
                                        }
                                        else
                                            continue;
                                    ...
                                    {
                                        if (ChiTietNTBocLaMia.Contains("new_loaigocmia"))
                                        {
                                            if (a["new_loaigocmia_vl"].ToString().IndexOf(((OptionSetValue)ChiTietNTBocLaMia["new_loaigocmia"]).Value.ToString()) == -1)
                                                continue;
                                        }
                                        else
                                            continue;
                                    ...
                                    {
                                        if (ChiTietNTBocLaMia.Contains("new_mucdichsanxuatmia"))
                                        {
                                            if (a["new_mucdichsanxuatmia_vl"].ToString().IndexOf(((OptionSetValue)ChiTietNTBocLaMia["new_mucdichsanxuatmia"]).Value.ToString()) == -1)
                                                continue;
                                        }
                                        else
                                            continue;
                                    ...
                                    traceService.Trace("Pass nhom giong mia");
                                    // NHom khach hang
                                    bool co = false;
                                    ...
                                    traceService.Trace("ds MHKN cua CSDT " + dsMHKN.Entities.Count());
                                    if (ctHDDTmia.Attributes.Contains("new_thamgiamohinhkhuyennong"))
                                    {
                                        traceService.Trace("CTHDDT mia co MHKN ");
                                    ...
                                    {
                                        traceService.Trace("CTHDDT mia KHONG co MHKN ");
                                        if (dsMHKN == null || dsMHKN.Entities.Count() == 0)
                                        {
                                            traceService.Trace("CSDT khong co MHKN");
                                            co = true;
                                        }
                                    }
                                    if (co == false)
                            ...
                            if (mCSDT != null && mCSDT.Id != Guid.Empty)
                            {
                                ...
                                decimal tongDM = tongDG * dientich;

                                traceService.Trace("dinhmucHL " + dongiaHL);
                                traceService.Trace("dinhmucKHL " + dongiaKHL);
                                traceService.Trace("tongDM " + tongDG);

                                en["new_dongia"] = new Money(tongDG);
                                en["new_dinhmuc"] = new Money(tongDM);
