                ...
                if (result.Entities.Count > 0)
                {
                    ...
                    if (vuDTId != null)
                    {
                        ...
                        if (dsCSDT != null && dsCSDT.Entities.Count() > 0)
                        {
                            foreach (Entity a in dsCSDT.Entities)
                            {
                                if (a.Contains("new_mucdichtuoi_vl"))  // Muc dich tuoi
                                {
                                    ...
                                    {
                                        if (a["new_mucdichtuoi_vl"].ToString().IndexOf(((OptionSetValue)CTNTTuoiMia["new_mucdichtuoi"]).Value.ToString()) == -1)
                                            continue;
                                    }
                                    else
                                    {
                                ...
                                if (dsVungDL.Entities.Count > 0)
                                {
                                    ...
                                    if (thuadatObj.Attributes.Contains("new_diachi"))
                                    {
                                        ...
                                        traceService.Trace("Lay entity dia chi " + diachi["new_path"].ToString());
                                        ...
                                        traceService.Trace("Query duoc vung dia ly hanh chinh ");

                                        foreach (Entity n in service.RetrieveMultiple(qe).Entities)
                                        {
                                            //traceService.Trace("Path vung dia ly hanh chinh " + n["new_path"].ToString());
                                ...
                                if (NTtuoimiaObj.Attributes.Contains("new_khachhangdoanhnghiepPPP"))
                                {
                                    Guid khId = NTtuoimiaObj.GetAttributeValue<EntityReference>("new_khachhangdoanhnghiep").Id;
                        ...
                        if (mCSDT != null && mCSDT.Id != Guid.Empty)
                        {
                            ...
                            upCTNTTuoi["new_chinhsachdautu"] = csdtKQEntity.ToEntityReference();
                            ...
                            upCTNTTuoi["new_dongiadinhmuchl"] = new Money(dinhmucHL);
                            upCTNTTuoi["new_dongiadinhmuckhl"] = new Money(dinhmucKHL);

                            upCTNTTuoi["new_thanhtiendmhl"] = new Money(thanhtienHL);
                            upCTNTTuoi["new_thanhtiendmkhl"] = new Money(thanhtienKHL);
                            ...
                            upCTNTTuoi["new_dagn_hoanlai"] = new Money(daGNHL);
                            upCTNTTuoi["new_dagn_khl"] = new Money(daGNKHL);
                            ...
                            if (denghiHL >=0)
                                upCTNTTuoi["new_dautuhl"] = new Money(denghiHL);

                            if(denghiKHL >= 0)
                                upCTNTTuoi["new_dautukhl"] = new Money(denghiKHL);

                            if(tongtiendt >= 0)
                                upCTNTTuoi["new_tongtiendautu"] = new Money(tongtiendt);
