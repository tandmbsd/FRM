...
namespace Plugin_ThemCSDTvaoNTThamCanh
{
    public class Plugin_ThemCSDTvaoNTThamCanh : IPlugin
    {
        ...
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
            ...
            {
                Entity NTThamcanh = (Entity)context.InputParameters["Target"];
                Guid entityId = NTThamcanh.Id;

                if (NTThamcanh.LogicalName == "new_danhgianangsuat")
                {
                    //traceService.Trace("Begin plugin");
                    if (context.MessageName.ToUpper() == "CREATE")
                    {
                        NTThamcanh = service.Retrieve("new_danhgianangsuat", entityId, new ColumnSet(new string[] { "subject", "createdon", "new_vudautu", "new_hopdongdautumia", "new_thuadatcanhtac", "new_loaidanhgia", "new_dientich" }));
