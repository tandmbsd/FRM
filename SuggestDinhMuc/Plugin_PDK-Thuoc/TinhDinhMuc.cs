...
namespace Plugin_PDK_Thuoc
{
    public class TinhDinhMuc
    {
        ...
        public void CalculateTrongMia()
        {
            ...
            if (pdk == null)
                throw new Exception(string.Format("Phiếu đăng ký thuốc '{0}' không tồn tại hoặc bị xóa!", pdkRef.Name));
            if (!pdk.Contains("new_hopdongdautumia"))
                throw new Exception(string.Format("Vui lòng chọn hợp đầu tư mía cho phiếu đăng ký thuốc '{0}'!", pdkRef.Name));
            string pdk_name = pdk.Contains("new_name") ? (" '" + pdk["new_name"].ToString() + "'") : "";

            if (!pdk.Contains("new_masophieudangky"))
                throw new Exception(string.Format("Phiếu đăng ký thuốc{0} chưa có mã. Vui lòng cập nhật mã!", pdk_name));
            ...
            QueryExpression query = new QueryExpression("new_thuadat_pdkthuoc");
            ...
            if (((OptionSetValue)pdk["new_apdung"]).Value == 100000000) // trồng và chăm sóc mía
            {
                ...
                foreach (Entity cthd in cthds.Entities)
                {
                    ...
                    Entity ct_pdk = new Entity("new_thuadat_pdkthuoc");
        ...
        private void sum_current_ct_pdk(EntityReference pdkRef, ref decimal hl, ref decimal khl)
        {
            ...
            StringBuilder fetch = new StringBuilder();
            fetch.AppendFormat("<fetch mapping='logical' aggregate='true' version='1.0'>");
            fetch.AppendFormat("<entity name='new_chitietdangkythuoc'>");
            ...
            fetch.AppendFormat("<filter type='and'>");
            fetch.AppendFormat("<condition attribute='new_phieudangkythuoc' operator='eq' value='{0}'/>", pdkRef.Id);
            fetch.AppendFormat("</filter>");
            fetch.AppendFormat("</entity>");
            fetch.AppendFormat("</fetch>");
            EntityCollection etnc = service.RetrieveMultiple(new FetchExpression(fetch.ToString()));
