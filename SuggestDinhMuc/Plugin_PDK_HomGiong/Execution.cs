...
namespace Plugin_PDK_HomGiong
{
    public class Execution : IPlugin
    {
        ...
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
            ...
            if (context.MessageName == "Create" && target.LogicalName == "new_phieudangkyhomgiong")
            {
            ...
            if (context.MessageName == "Create" && target.LogicalName != "new_phieudangkyhomgiong")
            {
                if (target.LogicalName == "new_chitietdangkyhomgiong" && target.Contains("new_phieudangkyhomgiong"))
                {
                    factory = (IOrganizationServiceFactory)serviceProvider.GetService(typeof(IOrganizationServiceFactory));
                    service = factory.CreateOrganizationService(context.UserId);
                    trace = serviceProvider.GetService(typeof(ITracingService)) as ITracingService;
                    ...
                    EntityReference pdkRef = (EntityReference)target["new_phieudangkyhomgiong"];
            ...
            else if (context.MessageName == "Update")
            {
                ...
                if (target.LogicalName == "new_chitietdangkyhomgiong")
                {
                    factory = (IOrganizationServiceFactory)serviceProvider.GetService(typeof(IOrganizationServiceFactory));
                    service = factory.CreateOrganizationService(context.UserId);
                    trace = serviceProvider.GetService(typeof(ITracingService)) as ITracingService;
                    ...
                    if (fullEntity.Contains("new_phieudangkyhomgiong") && fullEntity["new_phieudangkyhomgiong"] != null)
                    {
                        EntityReference pdkRef = (EntityReference)fullEntity["new_phieudangkyhomgiong"];
