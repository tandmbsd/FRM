...
function phuluchd() {
    ...
    var hopdongdautumia = Xrm.Page.getAttribute("new_hopdongdautumia");

        if (phuluchopdong.getValue() == null) {
            Xrm.Page.getControl("new_hopdongdautumia").setVisible(true);
            hopdongdautumia.setRequiredLevel("required");
        }
        else {
            Xrm.Page.getControl("new_hopdongdautumia").setVisible(false);
...
function phulucdaduyet() {
    ...
    if (phulucid != null) { // da duyet
        ...
        xml1.push("<filter type='and'>");
        xml1.push("<condition attribute='new_phuluchopdongid' operator='eq' uitype='new_phuluchopdong' value='" + phulucid.id + "'/>");
        xml1.push("</filter>");
        xml1.push("</entity>");
        xml1.push("</fetch>");
        CrmFetchKit.Fetch(xml1.join(""), false).then(function (rs) {
            ...
            if (rs.length > 0 && rs != null) {
                ...
                var tinhtrangduyet = rs[0].attributes.new_tinhtrangduyet.value;
                if (tinhtrangduyet == "100000005") {
