...
namespace Plugin_Sotiendenghi_PDKDichVu
{
    public class Plugin_Sotiendenghi_PDKDichVu : IPlugin
    {
        ...
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
            ...
            if (context.MessageName.ToUpper() == "CREATE")
            {
                ...
                Entity chitietpdkdichvu = service.Retrieve(target.LogicalName, target.Id, new ColumnSet(true));

                if (!chitietpdkdichvu.Contains("new_phieudangkydichvu"))
                {
                    throw new Exception("Chi tiết pdk dịch vụ không có phiếu đăng ký dịch vụ !!! ");
                }

                Entity pdkdichvu = service.Retrieve("new_phieudangkydichvu", ((EntityReference)chitietpdkdichvu["new_phieudangkydichvu"]).Id, new ColumnSet(true));
                ...
                List<Entity> listCTPDKDV = RetrieveMultiRecord(service, "new_chitietdangkyvattu", new ColumnSet(true), "new_phieudangkydichvu", pdkdichvu.Id);

                foreach (Entity ct in listCTPDKDV)
                {
                    ...
                    Entity ChiTietHD = RetrieveMultiRecord(service, "new_thuadatcanhtac"
                        , new ColumnSet(new string[] { "new_vutrong", "new_loaigocmia", "new_mucdichsanxuatmia", "new_giongmia"
                            , "new_thuadat", "createdon", "new_hopdongdautumia", "new_khachhang", "new_khachhangdoanhnghiep", "new_thamgiamohinhkhuyennong", "new_dientichthucte", "new_tuoimia", "new_dientichhopdong"
                            , "new_dinhmucphanbontoithieu", "new_copytuhddtthuedat", "new_trangthainghiemthu", "new_dachikhonghoanlai_homgiong", "new_dachihoanlai_homgiong", "new_tongchikhonghoanlai", "new_tongchihoanlai", "new_yeucaudacbiet" }), "new_thuadat"
                            , ((EntityReference)ct["new_thuadat"]).Id).FirstOrDefault();
                    ...
                    else
                    {
                        ...
                        if (mCSDT != null && mCSDT.Id != Guid.Empty)
                        {
                            ...
                            decimal dongiaDTKHL = ChiTietHD.Contains("new_dautukhonghoanlai") ? ((Money)ChiTietHD["new_dautukhonghoanlai"]).Value : 0;
                            ...
                            decimal dongiaDTHL = ChiTietHD.Contains("new_dautuhoanlai") ? ((Money)ChiTietHD["new_dautuhoanlai"]).Value : 0;
                            DMDTHL = dongiaDTHL * tyle / 100;
                        }

                        List<Entity> lstPhanbodautu = RetrieveMultiRecord(service, "new_phanbodautu", new ColumnSet(true), "new_thuacanhtac", ChiTietHD.Id);
                ...
                Entity newpdkvattu = new Entity("new_phieudangkydichvu");
                newpdkvattu.Id = pdkdichvu.Id;

                newpdkvattu["new_dinhmuc_khonghoanlai"] = new Money(TongDMDTKHL + (pdkdichvu.Contains("new_dinhmuc_khonghoanlai") ? ((Money)pdkdichvu["new_dinhmuc_khonghoanlai"]).Value : 0));
                newpdkvattu["new_dinhmuc_hoanlai_vattu"] = new Money(TongDMDTHLvattu + (pdkdichvu.Contains("new_dinhmuc_hoanlai_vattu") ? ((Money)pdkdichvu["new_dinhmuc_hoanlai_vattu"]).Value : 0));
                newpdkvattu["new_dinhmuc_hoanlai_tienmat"] = new Money(TongDMDTHL + (pdkdichvu.Contains("new_dinhmuc_hoanlai_tienmat") ? ((Money)pdkdichvu["new_dinhmuc_hoanlai_tienmat"]).Value : 0));

                newpdkvattu["new_giaingan_khonghoanlai"] = new Money(daGiainganKHL + (pdkdichvu.Contains("new_giaingan_khonghoanlai") ? ((Money)pdkdichvu["new_giaingan_khonghoanlai"]).Value : 0));
                newpdkvattu["new_giaingan_hoanlai_vattu"] = new Money(daGiainganHLvattu + (pdkdichvu.Contains("new_giaingan_hoanlai_vattu") ? ((Money)pdkdichvu["new_giaingan_hoanlai_vattu"]).Value : 0));
                newpdkvattu["new_giaingan_hoanlai_tienmat"] = new Money(0);

                newpdkvattu["new_denghi_khonghoanlai"] = new Money(DenghiKHL + (pdkdichvu.Contains("new_denghi_khonghoanlai") ? ((Money)pdkdichvu["new_denghi_khonghoanlai"]).Value : 0));
                newpdkvattu["new_denghi_hoanlai_vattu"] = new Money(DenghiHLvattu + (pdkdichvu.Contains("new_denghi_hoanlai_vattu") ? ((Money)pdkdichvu["new_denghi_hoanlai_vattu"]).Value : 0));

                service.Update(newpdkvattu);
            }
            #endregion
        }
