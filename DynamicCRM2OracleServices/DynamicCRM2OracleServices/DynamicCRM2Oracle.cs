...
{
    public partial class DynamicCRM2Oracle : ServiceBase
    {
        ...
        string pathLog = "D:\\test.log";
        public DynamicCRM2Oracle()
        {
            InitializeComponent();
            //eventLog1 = new System.Diagnostics.EventLog();
            //if (!System.Diagnostics.EventLog.SourceExists("DynamicCRM2Oracle.EventLog"))
            //{
            //    System.Diagnostics.EventLog.CreateEventSource(
            //        "DynamicCRM2Oracle.EventLog", "DynamicCRM2OracleLog");
            //}
            //eventLog1.Source = "DynamicCRM2Oracle.EventLog";
        ...
        protected override void OnStart(string[] args)
        {
            try
            {
                ...
                string host = settings["Host"] != null ? settings["Host"].Value : "";// xdoc.GetElementsByTagName("Host").Count > 0 ? xdoc.GetElementsByTagName("Host")[0].InnerText : "";
                string port = settings["Port"] != null ? settings["Port"].Value : "1521"; //xdoc.GetElementsByTagName("Port").Count > 0 ? xdoc.GetElementsByTagName("Port")[0].InnerText : "1521";
                string service = settings["Service"] != null ? settings["Service"].Value : "";//xdoc.GetElementsByTagName("Service").Count > 0 ? xdoc.GetElementsByTagName("Service")[0].InnerText : "";
                string user = settings["Username"] != null ? settings["Username"].Value : "";// xdoc.GetElementsByTagName("UserName").Count > 0 ? xdoc.GetElementsByTagName("UserName")[0].InnerText : "";
                string pass = settings["Password"] != null ? settings["Password"].Value : "";// xdoc.GetElementsByTagName("Password").Count > 0 ? xdoc.GetElementsByTagName("Password")[0].InnerText : "";
                ...
                WriteLog("host:" + host + " port:" + port + " service:" + service + "user:" + user + " pass:" + pass);
                Console.WriteLine("get config !");
                //eventLog1.WriteEntry("Get config!", EventLogEntryType.Information);
                ...
                //eventLog1.WriteEntry("Test connection OK!", EventLogEntryType.Information);
                var worker = new Thread(BeginETL);
                worker.Name = "ETLWork";
                worker.IsBackground = false;
                worker.Start();
        ...
        bool insertInvoice(List<Entity> data)
        {
            ...
            foreach (Entity a in data)
            {
                if (a.LogicalName.Trim().ToLower() == "new_etltransaction")
                {
                    ...
                    cmd.Parameters.Add(new OracleParameter("11", OracleDbType.Date, a.Contains("new_invoicedate") ? (DateTime?)((DateTime)a["new_invoicedate"]).AddHours(7) : null, ParameterDirection.Input));//Invoice_Date
                    cmd.Parameters.Add(new OracleParameter("12", OracleDbType.Date, a.Contains("new_gldate") ? (DateTime?)((DateTime)a["new_gldate"]).AddHours(7) : null, ParameterDirection.Input));// GL Date
                ...
                else
                {
                    ...
                    cmd.Parameters.Add(new OracleParameter("4", OracleDbType.Date, a.Contains("new_paymentdate") ? (DateTime?)a["new_paymentdate"] : null, ParameterDirection.Input)); //Payment_Date
                    cmd.Parameters.Add(new OracleParameter("5", OracleDbType.Varchar2, a.Contains("new_supplierbankname") ? a["new_supplierbankname"].ToString() : "", ParameterDirection.Input));//Bank_Account_Name
                    ...
                    cmd.Parameters.Add(new OracleParameter("16", OracleDbType.Varchar2, "", ParameterDirection.Input));//Reference_Num
